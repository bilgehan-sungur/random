        ...
        reference.child("Kullanıcılar").child(kadi).child("Sifre").setValue(pass).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()) {
                    ...
                    firebaseDatabase =FirebaseDatabase.getInstance();
                    reference=firebaseDatabase.getReference().child("Kullanıcı Özellikleri").child("ogrenci");
                    System.out.println(reference.getKey());
                    Map map = new HashMap();
                    map.put("resim","bos");
                    map.put("isim","bos");
                    ...
                    Toast.makeText(getApplicationContext(),"Kayıt olunamıyor..",Toast.LENGTH_LONG).show();
                }
            }
        });
    }
